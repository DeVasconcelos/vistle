...
bool StreamlineVtkm::GlobalData::getDataAtIndex(int index, std::vector<vistle::Object::const_ptr> &grids,
                                                ...
                                                viskores::cont::PartitionedDataSet &dataset)
{
    ...
    for (auto port = 0; port < this->inputFields.size(); port++) {
...
bool StreamlineVtkm::reduce(int timestep)
{
    ...
    if (!m_globalData.getDataAtIndex(static_cast<std::size_t>(timestep + 1), inputGrids, inputFields,
